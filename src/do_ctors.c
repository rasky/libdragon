...
{
	func_ptr * ctor_last = &__CTOR_END__ - 1;
	func_ptr * ctor_ptr = &__CTOR_LIST__;
	assertf((uint32_t)*ctor_ptr != 0xFFFFFFFF,
		"Invalid constructor sentinel.\nWhen linking with g++, please specify:\n   --wrap __do_global_ctors");
	while (++ctor_ptr <= ctor_last) {
...
void __wrap___do_global_ctors()
{
	...
	// thus the "-2".
	func_ptr * ctor_last = &__CTOR_END__ - 2;
	func_ptr * ctor_ptr = &__CTOR_LIST__;
	...
	assertf(
		(uint32_t)*ctor_ptr == 0xFFFFFFFF && (uint32_t)*(&__CTOR_END__ - 1) == 0x0,
