...
static void wave_read(void *ctx, samplebuffer_t *sbuf, int wpos, int wlen, bool seeking) {
	...
	extern void raw64_decoder_read(samplebuffer_t *sbuf, int base_rom_addr, int wpos, int wlen, int bps);
	raw64_decoder_read(sbuf, samp->data8_offset, wpos, wlen, samp->bits >> 4);
}
...
void xm64player_open(xm64player_t *player, const char *fn) {
	...
	// Load the XM context
	int sample_rate = mixer_sample_rate();
