...
static void ym_wave_read(void *ctx, samplebuffer_t *sbuf, int wpos, int wlen, bool seeking) {
	...
	// Computer the number of samples per audioframe. Keep it as floating point
	// for higher precision in calculating the mapping between sample numbers
	// and audioframes.
	float f_samples_per_vbl = player->wave.frequency / player->playfreq;
	...
	if (seeking && !player->decoder) {
		player->curvbl = ((float)wpos / f_samples_per_vbl);
		dfs_seek(player->fh, player->start_off + player->curvbl * 16, SEEK_SET);
	}
	...
	int lastvbl = ((float)(wpos+wlen-1) / f_samples_per_vbl);

	// Now switch to integers. Number of audioframes to process, and number
	// of (integer) samplers per audioframe.
	int nvbl = lastvbl - player->curvbl + 1;
	int samples_per_vbl = (int)f_samples_per_vbl;

	// Get the pointer to the sample buffer.
	int16_t *samples = samplebuffer_append(sbuf, nvbl*samples_per_vbl);

	int16_t *out = samples;
	const int num_channels = AY8910_OUTPUT_STEREO ? 2 : 1;

	for (int i=0;i<nvbl;i++) {
		...
		// sample buffer.
		ay8910_gen(&player->ay, out, samples_per_vbl);
		out += (int)samples_per_vbl * num_channels;
		player->curvbl++;
	}
}
...
void ym64player_open(ym64player_t *player, const char *fn, ym64player_songinfo_t *info) {
	...
	float freq = (float)player->extfreq / 8 / AY8910_DECIMATE;
	...
	int len = (float)player->nvbl * freq / player->playfreq;
	...
	player->first_ch = -1;
	debugf("ym64: loading %s (freq:%ld, wfreq:%ld)\n", fn, player->extfreq/8, player->extfreq/8/AY8910_DECIMATE);
}
...
void ym64player_play(ym64player_t *player, int first_ch) {
	...
	mixer_ch_set_pos(first_ch, (float)player->curvbl * player->wave.frequency / player->playfreq);
}
...
void ym64player_duration(ym64player_t *player, int *len, float *secs) {
	if (len) *len = player->nvbl;
	if (secs) *secs = (float)player->nvbl / (float)player->playfreq;
}

void ym64player_tell(ym64player_t *player, int *pos, float *secs) {
	if (pos) *pos = player->curvbl;
	if (secs) *secs = (float)player->curvbl / (float)player->playfreq;
}
