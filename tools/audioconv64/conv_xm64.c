...
int xm_convert(const char *infn, const char *outfn) {
	...
	xm_context_t* ctx;
	xm_create_context_safe(&ctx, xmdata, fsize, 44100);
	...
	if (flag_verbose) {	
		fprintf(stderr, "  * ROM size: %u Kb (samples:%lu)\n",
			romsize / 1024, mem_sam / 1024);
		fprintf(stderr, "  * RAM size: %lu Kb (ctx:%lu, patterns:%u, samples:%u)\n",
	...
	int ret = xm_context_load(&ctx2, out, 44100);
