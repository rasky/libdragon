...
void print_args( char * name )
{
    ...
    fprintf(stderr, "   -s/--slices <h,v>   Specify sprite slices (default: auto)\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Supported formats: AUTO, RGBA32, RGBA16, CI8, I8, IA8, CI4, I4, IA4\n");
    fprintf(stderr, "NOTE: this tool will not quantize the input image\n");
}
...
int convert(const char *infn, const char *outfn, tex_format_t outfmt, int hslices, int vslices) {
...
int main(int argc, char *argv[])
{
    ...
    if ((argc == 4 || argc == 6) && (!strcmp(argv[1], "16") || !strcmp(argv[1], "32"))) {
        ...
        infn = argv[i++];
        outfn = argv[i++];
        printf("WARNING: old command-line syntax was used, please switch to new syntax\n");
        return convert(infn, outfn, outfmt, hslices, vslices);
    }
    ...
    for (int i = 1; i < argc; i++) {
        if (argv[i][0] == '-') {
            ...
            } else if (!strcmp(argv[i], "-s") || !strcmp(argv[i], "--slices")) {
                if (++i == argc) {
                    fprintf(stderr, "missing argument for --slices\n");
                    return 1;
                }
                char extra;
                if (sscanf(argv[i], "%d,%d%c", &hslices, &vslices, &extra) != 2) {
                    fprintf(stderr, "invalid argument for --slices: %s\n", argv[1]);
                    return 1;
                }
            } else {
        ...
        if (flag_verbose)
            printf("Converting: %s -> %s [fmt=%s slices=%d,%d]\n",
                infn, outfn, tex_format_name(outfmt), hslices, vslices);
        if (convert(infn, outfn, outfmt, hslices, vslices) != 0)
            error = true;
        free(outfn);
    }
